...
## Local EMS Prometheus profile

The local deployment in this repository uses the following Prometheus settings:

- `prometheus.prometheusSpec.replicas: 2` for HA.
- `prometheus.prometheusSpec.storageSpec.volumeClaimTemplate` with `storageClassName: local-path` and `5Gi` per replica.
- `prometheus.prometheusSpec.nodeSelector.node-group.ntels.io/type: upm-ems` to place Prometheus on EMS nodes only.
- `prometheus.prometheusSpec.tolerations` with `NoExecute` and `tolerationSeconds: 10` for fast node eviction on `not-ready` and `unreachable`.
- `prometheus.prometheusSpec.topologySpreadConstraints` with `maxSkew: 1` and `topologyKey: kubernetes.io/hostname`.
- `prometheus.prometheusSpec.podAntiAffinity: hard` to avoid placing both replicas on the same node.
- `prometheus.prometheusSpec.updateStrategy.type: RollingUpdate` with `maxUnavailable: 1`.

## Local EMS Alertmanager profile

The local deployment in this repository uses the following Alertmanager settings:

- `alertmanager.alertmanagerSpec.replicas: 2` for HA clustering.
- `alertmanager.alertmanagerSpec.storage.volumeClaimTemplate` with `storageClassName: local-path` and `2Gi` per replica.
- `alertmanager.alertmanagerSpec.nodeSelector.node-group.ntels.io/type: upm-ems` to place Alertmanager on EMS nodes only.
- `alertmanager.alertmanagerSpec.tolerations` with `NoExecute` and `tolerationSeconds: 10` for fast node eviction on `not-ready` and `unreachable`.
- `alertmanager.alertmanagerSpec.topologySpreadConstraints` with `maxSkew: 1` and `topologyKey: kubernetes.io/hostname`.
- `alertmanager.alertmanagerSpec.podAntiAffinity: hard` to avoid placing both replicas on the same node.
- `alertmanager.alertmanagerSpec.updateStrategy.type: RollingUpdate` with `maxUnavailable: 1`.
- `alertmanager.alertmanagerSpec.clusterLabel`, `clusterGossipInterval`, `clusterPeerTimeout`, and `clusterPushpullInterval` to make the gossip cluster settings explicit.

## Local EMS Grafana profile

The local deployment in this repository uses the following Grafana settings:

- `grafana.replicas: 1`.
- `grafana.persistence.enabled: true` with `type: sts`, `storageClassName: local-path`, and `size: 2Gi`.
- `grafana.initChownData.enabled: false` to avoid `init-chown-data` failures on local-path volumes and rely on `fsGroup`.
- `grafana.nodeSelector.node-group.ntels.io/type: upm-ems` to place Grafana on EMS nodes only.
- `grafana.tolerations` with `NoExecute` and `tolerationSeconds: 10` for fast node eviction on `not-ready` and `unreachable`.
- `grafana.deploymentStrategy.type: RollingUpdate`.
- `grafana.service.type: NodePort` with `grafana.service.nodePort: 30906`.

## Local EMS Node Exporter profile

The local deployment in this repository uses the following Node Exporter scheduling rule:

- `prometheus-node-exporter.affinity.nodeAffinity.requiredDuringSchedulingIgnoredDuringExecution` with `node-group.ntels.io/type In [upm-app, upm-ems]`.
- `nodeSelector` is not used here because it cannot express an OR condition across multiple label values.

## Local EMS kube-state-metrics profile

The local deployment in this repository uses the following kube-state-metrics settings:

- `kube-state-metrics.nodeSelector.node-group.ntels.io/type: upm-ems`.
- `kube-state-metrics.tolerations` with `NoExecute` and `tolerationSeconds: 10` for fast node eviction on `not-ready` and `unreachable`.
- `kube-state-metrics.affinity.podAntiAffinity.requiredDuringSchedulingIgnoredDuringExecution` with `topologyKey: kubernetes.io/hostname`.
- `kube-state-metrics.updateStrategy: RollingUpdate`.

## Local EMS Prometheus Thanos sidecar profile

The local deployment in this repository uses the following Prometheus Thanos sidecar settings:

- `prometheus.prometheusSpec.thanos.image: quay.io/thanos/thanos:v0.39.2` to run one Thanos sidecar per Prometheus replica.
- `prometheus.thanosService.enabled: true` to expose a headless discovery service for the sidecars.
- `prometheus.thanosServiceMonitor.enabled: true` to scrape sidecar metrics.
- `prometheus.prometheusSpec.externalLabels.cluster: upm-ems` as a shared cluster label.
- `prometheus.prometheusSpec.replicaExternalLabelName: replica` for Thanos dedup across `prometheus-0` and `prometheus-1`.
- `prometheus.prometheusSpec.prometheusExternalLabelName: prometheus` for source identification in Thanos Query.
